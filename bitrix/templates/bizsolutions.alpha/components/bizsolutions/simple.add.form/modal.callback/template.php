...
if (strlen($arResult['MESSAGE']) > 0) {
    ...
    <div class="modal-footer">
        <div class="row">
            <div class="col-xs-6 col-xs-offset-3">
                <button type="button" class="btn-u btn-block" data-dismiss="modal"><?= GetMessage('BS_CLOSE_TITLE') ?></button>
            </div>
        </div>
    </div>
    <?
    ...
    <form id="callForm" class="modal-form" action="<?= POST_FORM_ACTION_URI ?>" method="post">
        ...
        <div class="modal-header">
            ...
            <h4 class="modal-title"><?= GetMessage('BS_MODAL_TITLE') ?></h4>
        </div>
            ...
            <div class="row">
                ...
                <? foreach ($arResult['FORM_FIELDS'] as $propId => $arField) {
                    ...
                    if ($arField['IS_REQUIRED'] == 'Y') {
                        ...
                        $validator .= ' data-bv-notempty-message="' . GetMessage('BS_ERROR_REQUIRED', array('#FIELD_NAME#' => $arField['NAME'])) . '"';
                    }
                    if ($arField['CODE'] == 'USER_PHONE') {
                        $validator .= ' data-bv-phone="true"';
                        $validator .= ' data-bv-phone-country="RU"';
                        $validator .= ' data-bv-phone-message="' . GetMessage('BS_ERROR_PHONE') . '"';
                    }
                    if ($arField['CODE'] == 'USER_EMAIL') {
                        $validator .= ' data-bv-emailaddress="true"';
                        $validator .= ' data-bv-emailaddress-message="' . GetMessage('BS_ERROR_EMAIL') . '"';
                    }
                        ...
                        <div>
                            ...
                            for ($i = 0; $i < $inputNum; $i++) {
                                ...
                                if ($arField['CUSTOM_TYPE'] == 'text') {
                                    ...
                                    ?>
                                    <div class="form-group has-feedback">
                                        <textarea class=" <?= ToLower($arField['CODE'])?> form-control resize-vertical margin-bottom-20" name="PROPERTY[<?= $propId ?>][<?= $i ?>]" id="PROPERTY_<?= $propId ?>" rows="3"<?= $validator ?>><?= $value ?></textarea>
                                    </div>
                                    <?
                                }
                            } ?>
                ...
                if ($arParams['USE_CAPTCHA'] == 'Y') {
                    // Validation
                    $validator = ' data-bv-notempty="true"';
                    $validator .= ' data-bv-notempty-message="' . GetMessage('BS_ERROR_CAPTCHA') . '"';
                    ?>
                    <div class="col-md-12">
                        <div>
                            <label><?= GetMessage('BS_CAPTCHA_1') ?></label>
                            <div class="form-group has-feedback text-center captcha">
                                <a href="" class="tooltips" data-toggle="tooltip" data-placement="top" data-original-title="<?= GetMessage('BS_CAPTCHA_2') ?>">
